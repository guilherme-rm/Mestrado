...
def test_episode_and_step_csv(tmp_path, monkeypatch):
    ...
    for ep in range(2):
        ep_logger.log(episode=ep, steps=10, avg_reward=0.5, qos_rate=0.8, capacity_mean=5.0, epsilon_last=0.3, duration_seconds=1.2)
    for step in range(5):
        step_logger.log(global_step=step, episode=0, step_in_episode=step, epsilon=0.1, mean_reward=0.2, qos_mean=0.9, capacity_sum_mbps=3.3)
    ep_logger.close(); step_logger.close()
